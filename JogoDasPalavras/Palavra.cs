...
namespace JogoDasPalavras
{
    public class Palavra
    {
        ...
        private List<string> palavrasParaSortear = new List<string>
        {
    ...
    "jovem", "juíza", "juizo", "julho", "junho", "jurar", "justa"
        ...
        private char[]? PopularLetrasEncontradas(int length)
        {
            char[] letrasEncontradas = new char[length];
        ...
        {
            bool validarPalavra = false;
            
            foreach (string palavra in palavrasParaSortear)
            {
                 validarPalavra=  String.Compare(palpiteJogador.ToLower(), palavra, CultureInfo.CurrentCulture, CompareOptions.IgnoreNonSpace) == 0;
            }

            if(validarPalavra == false)
            {
